...
class processForm
{
    public function signup($conn, $globalObj)
    {
        ...
        {
            // Set Variables
            $fullname = $conn->escapeValues(ucwords(strtolower($_POST['fullname'])));
            $email = $conn->escapeValues(strtolower($_POST['email']));
            $username = $conn->escapeValues(strtolower($_POST['username']));
